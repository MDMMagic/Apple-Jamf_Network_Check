...
run_test "Apple Push Notification Service" "gateway.push.apple.com"          2195 TCP "APNs legacy push gateway (Jamf server → Apple)"
run_test "Apple Push Notification Service" "feedback.push.apple.com"         2196 TCP "APNs feedback service (Jamf server)"
run_test "Apple Push Notification Service" "gateway.sandbox.push.apple.com"  2195 TCP "APNs sandbox (dev/test Jamf environments)"
...
if [[ $FAIL -gt 0 ]]; then
    ...
    <ul>
      ...
      <li>APNs requires <strong>port 5223</strong> in addition to 443 — many firewalls block this. Ensure <code>*.push.apple.com</code> on TCP 5223 is permitted.</li>
      <li>JCDS uses Amazon CloudFront and S3 — ensure <code>*.cloudfront.net</code> and <code>*.s3.amazonaws.com</code> on 443 are reachable.</li>
...
<head>
  ...
  <title>Jamf Network Test Report — $RUN_DATE</title>
  <style>
...
<header>
  <div>
    ...
    <div class="meta">Host: ${HOSTNAME_LOCAL} &nbsp;·&nbsp; ${RUN_DATE} &nbsp;·&nbsp; Timeout: ${TIMEOUT}s per test</div>
  </div>
</header>
...
<footer>
  Generated by jamf_network_test.sh &nbsp;·&nbsp;
